...
Esta pasta contém componentes reutilizáveis que foram extraídos dos componentes
específicos do ranking para evitar duplicação de código.
...
```tsx
<Section title='Título da Seção'>{/* Conteúdo da seção */}</Section>
```
...
```tsx
<AnimatedContainer>{/* Conteúdo animado */}</AnimatedContainer>
```
