      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          <div className="flex items-center gap-3">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-red-500/15 px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-red-500">
                ...
                {[
                  ...
                  <div key={u.ini} className="relative aspect-video rounded-lg border border-border bg-muted/40 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-1">
                      <span className="flex h-10 w-10 items-center justify-center rounded-full font-mono text-xs font-bold text-background" style={{ backgroundColor: 'var(--brand, #d9f99d)' }}>
                        {u.ini}
                      </span>
              ...
              <div className="flex gap-2">
                {['Drilling', 'Quiz', 'Flashcards'].map((t, i) => (
                  <span
                    ...
                    style={i === 1 ? { backgroundColor: 'var(--brand, #d9f99d)' } : undefined}
                  >
                    {t}
                  </span>
                ))}
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                {options.map((o) => (
                  <span
                    ...
                    style={o === 'goes' ? { borderColor: 'var(--brand, #d9f99d)', backgroundColor: 'color-mix(in srgb, var(--brand, #d9f99d) 10%, transparent)' } : undefined}
                  >
                    {o}
                  </span>
                ))}
          ...
          <div className="flex flex-col gap-px bg-border/60">
            <div className="bg-card p-5">
              <p className="flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                <MessageSquare className="w-3 h-3" style={{ color: 'var(--brand, #d9f99d)' }} /> Chat
              </p>
              <div className="mt-3 space-y-2">
                ...
                <div className="ml-auto max-w-[85%] rounded-lg rounded-tr-sm px-3 py-2 text-[13px] text-background" style={{ backgroundColor: 'var(--brand, #d9f99d)' }}>got it</div>
              </div>
            </div>
            <div className="bg-card p-5 flex-1">
              <p className="flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                <NotebookPen className="w-3 h-3" style={{ color: 'var(--brand, #d9f99d)' }} /> Teacher notes
              </p>
